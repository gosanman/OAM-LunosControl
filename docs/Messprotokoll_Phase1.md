...
U2 steht damit nach dem Anlegen von 12 V auf Stillstand, bevor der RP2040 etwas
getan hat.

> **Dieser Gewinn wird nicht eingelöst.** Er kostete den Nachbarchip U3 seinen
> Abgleich (Befund B1), und ein `store 59` träfe U2 auf dieselbe Weise. Auf einem
> Bus mit zwei GP8413 ist der Store im eingebauten Zustand nicht sicher; die
> Entscheidung vom 25.09.2026 lautet deshalb: **kein Store**.
...
Beobachtet: **5,013 V** an S1 (derselbe Durchgang wie P10, ohne jeden I²C-Zugriff).
Im EEPROM stand der Halbausschlag `0x8000`; ein halber Skalenwert ergibt nur im
0–10-V-Bereich 5 V, im 0–5-V-Bereich wären es 2,50 V gewesen.

**Nicht als bestanden eingetragen**, siehe den Hinweis unten: die Beobachtung
unterscheidet nicht zwischen „Bereichsbit mitgesichert" und „Werkseinstellung ist
ohnehin 0–10 V". Nach der Entscheidung gegen den Store (B1) ist die Frage für den
Betrieb ohnehin zweitrangig — es gibt keinen gespeicherten Einschaltwert mehr, den
ein falscher Bereich verfälschen könnte.
...
| P7 Skalenfehler | reiner Steigungsfehler, Faktoren 9975 / 9973 / (9994) / (9948) | 25.09.2026 |
| P15 Isolator | Sequenz kommt an, Auswertung offen — und der Store ist verworfen | 25.09.2026 |
| P10 EEPROM-Wert | bestanden für U2, um den Preis von U3 (Befund B1) | 25.09.2026 |
| P11 Bereichsbit | Beobachtung spricht dafür, nicht eingetragen | 25.09.2026 |

**Die Entscheidung aus B1 überlagert alles andere: kein Store.** Damit gibt es
keinen EEPROM-Einschaltwert, und nach jedem Einschalten liegt an allen Kanälen der
Werkswert **0 V — Volllast in einer Richtung**, bis `setup()` den sicheren Zustand
schreibt. Fehlt der KNX-Bus bei anliegenden 12 V, bleibt es dabei.

> Die Faktoren für **S3 und S4 sind für Platine 1 hinfällig**: B1 hat den Abgleich
> von U3 um den Faktor 1,105 verstellt. Sie gelten nur als Beleg dafür, wie die
> Kanäle vor dem Store waren.

### Offen

- [ ] **Kein `store` mehr** auf dieser oder einer weiteren Platine, bis B1 geklärt
      ist. An S3/S4 von Platine 1 dürfen keine Lüfter.
- [ ] Nachziehen: PLAN Phase 5, CLAUDE.md Invariante 7, ETS-Parameter
      „Einschaltwert ohne Bus (EEPROM)" samt Hilfetext, Applikationsbeschreibung.
- [ ] Startup-Pfad prüfen: wie lange liegen nach dem Einschalten 0 V an, bevor
      `KwlOutput::begin()` den sicheren Zustand schreibt? Heute läuft das erst nach
      `openknx.init()`.
